...
## Second update — same day, 2026-08-03 (Stage 3 first pass: identity mapping)

Also shipped and deployed to production (git commit `f75135a`; Vercel deployment `dpl_GBJwPq5Z3N44gzHMx11ZWvA2WkJi`, aliased to `ops.jointx.co.za`; same file-isolation deploy procedure as above):

- **"Map identity" action** (tag icon) on each Stock row. Walks a reviewer (tenant owner/admin, or app admin) through assigning an internal product/variant (e.g. `JET` / `Black` / `XL`) and an exact supplier product/variant (e.g. `Daniel Slaves 220gsm Tee` / `Black` / `XL` / supplier SKU), creating whichever of those don't exist yet inline. Does not touch `public.inventory` or `current_stock`; only records identity.
- **New migration `202608030002_inventory_reviewer_mapping_rpc.sql`**: adds `public.inventory_reviewer_map_legacy_item(...)`, applied directly via Studio SQL editor and verified (function exists, `authenticated` has EXECUTE, `inventory` row count unchanged at 42). Architectural note worth knowing: the originally-deployed Phase 1 design (migrations 03–06) routed mapping *suggestions* through a `service_role`-only import function and mapping *decisions* through a separate authenticated RPC that required an existing suggestion to already exist — meaning there was no path for a human to map a row interactively through the app, since nothing had ever staged a suggestion. This new RPC combines suggest+approve into one step for the real one-person-does-both workflow, reusing the exact same `inventory_can_review_tenant()` authorization and hierarchy-compatibility checks as the original `inventory_decide_legacy_mapping()`.
- **Stock tab List/Grouped view toggle.** Grouped mode clusters rows by their mapped internal product (via the existing `inventory_legacy_compat_v` view) with a "Needs mapping" group first for everything unmapped. Deliberately does **not** do fuzzy/automatic name-based grouping — the legacy names have real typos and unit variants (`220gms` vs `220 gms` vs `220gsm` vs `230gms` are not all the same product) that would make silent automatic grouping unsafe.
- No code writes to `inventory_movements.related_order_id`/`related_purchase_order_id` yet — that's the deferred order-linking idea, see project memory `project_opps_inventory_phase1.md`.

## Third update — same day, 2026-08-03 (mapping form usability pass)

Also shipped and deployed (git commits `b6c7b5c`, `254c1f8`, `ae634a8`; final Vercel deployment `dpl` behind `joint-x-opps-kh3uphnbp`, aliased to `ops.jointx.co.za`). No new migration — pure frontend.

Triggered by real usage: testing the mapping dialog on `Custom Labels` (a mistaken/test row) surfaced that free-text colour/size fields let a reviewer type `black, Blue` / `S, M` as one value, silently creating one variant literally named that instead of the several variants it actually represents — since one mapping can only point one legacy row at one internal variant, that data would have been wrong.

Fixes and additions:
- New-identity name fields (`Internal product name`, `Supplier product code`) no longer default to the raw legacy text — they start blank so a reviewer has to deliberately type the clean identity instead of clicking through with the messy source name copied over.
- Colour/size entry is now single-select preset chips (`COLOUR_PRESETS`/`SIZE_PRESETS` in `Inventory.jsx`) with a "Custom…" fallback — structurally prevents the comma-list mistake rather than just warning about it. A "Not a garment? Use 'Standard'" shortcut handles non-apparel rows (labels, packaging, ink, etc.) that still need non-blank values to satisfy the schema.
- All four identity pickers are now a searchable typeahead (new generic `SearchSelect` component) instead of a plain `<select>`, matching the existing suggestion-as-you-type pattern used for client search in order creation (`src/components/orders/NewOrderDrawer.jsx` — note: that pattern is hand-rolled, not a shared component, so `SearchSelect` is a new, more generic implementation, not a literal reuse).
- Supplier product auto-selects when exactly one already exists for the chosen supplier + internal product, since most real stock reuses the same supplier product repeatedly.
- New third Stock view mode, **Gallery**: image-card grid using category-representative stock photos, since `public.inventory` rows have no photo of their own (no `image_url` column exists — only the Shop Catalog's `products` table has real images). Categories without a proven-safe existing fallback image show a generic package icon rather than an unverified URL.

Still outstanding from the user's last round of feedback, not yet built: deeper prefill intelligence beyond the one supplier-product auto-select (e.g. suggesting a whole internal product + variant set from history), and no bulk "create all size/colour combinations for a product at once" tool — the chip picker is still one variant per mapping action by design, since one legacy row is one physical stock count.

## Fourth update — same day, 2026-08-03 (variant-aware movements, quantity rollups, stock photos)

Also shipped and deployed (git commit `a50ec7c`; Vercel deployment behind `joint-x-opps-4fomatxrq`, aliased to `ops.jointx.co.za`). Two new migrations, both applied and verified.

- **`inventory_movements` now snapshots the variant.** Migration `202608030004` added `inventory_product_id`/`inventory_variant_id`/`inventory_supplier_product_id`/`inventory_supplier_variant_id` (all nullable). The Count Stock dialog now records the row's *current approved mapping at the moment of the count* into those columns, and shows an amber "not mapped yet" notice when there's nothing to record. This was the direct answer to "when recording stock movement need to know what variant was moved" — motivated by the real example: JET 220gsm comes from two suppliers (Daniel Alves — more cotton feel — and DH Cotton Tees — poly blend), and movements need to say which one moved, not just "JET moved."
- **Grouped Stock view now shows real totals**, not just a row count: per internal product, total quantity across all its supplier variants, plus a breakdown line like `Daniel Alves 220gsm Tee: 50 · DH Cotton Tees: 50`. Computed client-side from `inventory_legacy_compat_v` (already deployed), no new view needed — directly answers "how many JETs do we have — 50 DA and 50 DH."
- **Stock photos.** Migration `202608030003` added `inventory_images` (one row per photo, private-upload ref into the existing tenant-scoped `uploads` bucket, any tenant member can add/remove). Edit Item dialog has a Photos section (multi-upload + delete). Clicking a thumbnail anywhere (List, Grouped, or Gallery view) opens a full-screen lightbox with prev/next across all of that item's photos. Gallery cards use the item's own first real photo when one exists, falling back to the category-representative stock photo otherwise.
- **Naming principle confirmed by the user**: the internal product name should be whatever the garment's own neck label says — that's how staff physically identify it — not a copy of the supplier's messy product text.
- **Open naming question, not yet resolved**: the user has written the Daniel [Alves/Alaves] supplier name two different ways across two messages, which is a live example of exactly the identity-mapping problem this whole project exists to fix. Confirm the one correct spelling before it gets typed into any `official_product_name`/`suppliers.name` field.

## Fifth update — same day, 2026-08-03 (real bugs found while starting to map JET)

Confirmed spelling: **Daniel Alaves**. Deployed fixes (git commit `f802bd5`, no new migration):

- **Real bug, root cause of "multiple errors" while saving a mapping**: the mapping mutation created each missing identity level (internal product → variant → supplier product → supplier variant) but never committed the new id back into component state or invalidated its query until the *whole* mutation finished. If a later step failed, the dialog still showed "+ Create new" for levels that had actually already succeeded — retrying tried to create `JET` a second time and hit the unique `internal_code` constraint. Fixed: each successful sub-step now commits its id into state and invalidates its query immediately, so a retry after a partial failure resumes instead of repeating.
- **Real bug**: the auto-generated internal SKU used the locally-typed "new product code" field even when reusing an *existing* internal product (always empty in that case) — produced SKUs like `SKU-Red-2XL` instead of `JET-Red-2XL`. Fixed to look up the actual selected product's `internal_code`.
- **`Daniel Slaves 220gms Tees / Brown car key -M`** confirmed by the user as test/mistake data — needs archiving (not deleting, matching how the other test rows were handled), not treated as a real colour question.
- **New**: Suppliers page cards now show a dynamic "Mapped stock items" section — pulls live from `inventory_supplier_products`/`inventory_supplier_variants` filtered by that supplier, separate from the pre-existing manually-typed "Supplier items" JSONB list on the Supplier record. Updates automatically as more rows get mapped from Inventory, per the user's request to see supplier-linkage reflected on the supplier's own profile.

**JET 220gsm mapping worksheet** (12 rows, 17 pieces, excluding the "Brown car key" test row): recorded in this session's conversation, not yet re-copied into this file — if resuming cold, ask the user or re-derive from `public.inventory` rows matching `name ilike '%220g%tee%'` under the Daniel-Alaves-typo family.
