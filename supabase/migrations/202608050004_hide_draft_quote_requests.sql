-- X LAB's Account page can now save a "Start an order" request as a draft
-- (client_quote_requests.status = 'draft') instead of only ever submitting
-- it for review. get_internal_client_requests_unscoped had no status filter
-- on the quote_requests branch, so a client's private, unsent draft would
-- otherwise show up in Client Requests as if it were a real submission.
-- This keeps the rest of the function (renamed from get_internal_client_requests
-- by 202606200007_tenant_xlab_request_rpcs.sql) byte-for-byte identical and
-- only excludes drafts from the quote_requests branch.
...
-- 202606270004 revoked PUBLIC/anon/authenticated execute on the unscoped
-- function; CREATE OR REPLACE does not reset grants, but re-assert it here
-- so this migration is correct standalone regardless of run order.
revoke execute on function public.get_internal_client_requests_unscoped(text, text, text, text, int) from public, anon, authenticated;
