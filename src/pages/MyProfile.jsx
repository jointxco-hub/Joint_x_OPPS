...
// Phase 2B: previously wrote directly to public.users
// (`supabase.from("users").update({preferred_name}).eq("user_email", ...)`)
// - under Phase 2A's tightened UPDATE policy (is_app_admin() only) that
// silently affected 0 rows for every ordinary user, so "Preferred name
// saved" was lying. Now goes through dataClient.auth.updateMe(), which
// itself goes through the narrow update_my_opps_profile() RPC - no
// direct public.users write from this page at all.
export default function MyProfile() {
  ...
  const save = async () => { if (!user?.email || !name.trim()) return; setSaving(true); try { await dataClient.auth.updateMe({ full_name: name.trim(), preferred_name: name.trim() }); const updated = await dataClient.auth.me(); setUser(updated); toast.success("Preferred name saved"); } catch (error) { toast.error(error?.message || "Could not save preferred name"); } finally { setSaving(false); } };
