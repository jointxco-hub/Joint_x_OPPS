...
export default function FileManager() {
  ...
  const uploadFiles = async (e) => {
    ...
    try {
      for (const file of files) {
        ...
        await dataClient.entities.ClientAsset.create({
          title: file.name,
          file_url,
  ...
  const startLongPress = (folder) => {
    longPressTimer.current = setTimeout(() => setLongPressFolder(folder), 500);
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-5xl mx-auto px-4 py-6 md:py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <label className="cursor-pointer">
            <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-border bg-card text-sm font-medium text-foreground hover:bg-secondary transition-all shadow-apple-sm ${uploading ? "opacity-60 pointer-events-none" : ""}`}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {visibleFolders.map((folder) => {
                ...
                return (
                  <div
                    key={folder.id}
                    className="group bg-card rounded-2xl border border-border shadow-apple-sm overflow-hidden select-none"
                    onTouchStart={() => startLongPress(folder)}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-end gap-1 px-3 py-2 opacity-0 group-hover:opacity-100 transition-opacity md:flex hidden">
                      <button onClick={() => setFolderForm(folder)} className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary transition-all" title="Rename">
                        <Pencil className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => archiveFolder.mutate(folder.id)} className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary transition-all" title="Archive">
                        <Archive className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => setDeleteConfirm({ type: "folder", item: folder })} className="p-1.5 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all" title="Delete">
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    </div>
                    {/* Mobile: always-visible 3-dot button */}
                    <div className="flex items-center justify-end px-3 py-2 md:hidden">
                      <button onClick={() => setLongPressFolder(folder)} className="p-1.5 rounded-lg text-muted-foreground hover:bg-secondary transition-all">
      ...
      {/* Long-press / mobile folder actions sheet */}
      {longPressFolder && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 backdrop-blur-sm" onClick={() => setLongPressFolder(null)}>
          <div className="bg-card rounded-t-3xl border-t border-border w-full max-w-sm pb-safe" onClick={e => e.stopPropagation()}>
            <div className="w-10 h-1 bg-border rounded-full mx-auto mt-3 mb-4" />
            <p className="text-sm font-semibold text-foreground px-5 pb-3 border-b border-border truncate">{longPressFolder.name}</p>
            <div className="p-3 space-y-1">
              <button
                onClick={() => { setFolderForm(longPressFolder); setLongPressFolder(null); }}
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm text-foreground hover:bg-secondary transition-all text-left"
              >
                <Pencil className="w-4 h-4 text-muted-foreground" /> Rename
              </button>
              <button
                onClick={() => { archiveFolder.mutate(longPressFolder.id); setLongPressFolder(null); }}
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm text-foreground hover:bg-secondary transition-all text-left"
              >
                <Archive className="w-4 h-4 text-muted-foreground" /> Archive
              </button>
              <button
                onClick={() => { setDeleteConfirm({ type: "folder", item: longPressFolder }); setLongPressFolder(null); }}
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm text-destructive hover:bg-destructive/10 transition-all text-left"
              >
                <Trash2 className="w-4 h-4" /> Delete
              </button>
            </div>
            <div className="p-3 pt-0">
              <button onClick={() => setLongPressFolder(null)} className="w-full py-3 rounded-xl bg-secondary text-sm font-medium text-foreground">
                Cancel
              </button>
            </div>
          </div>
