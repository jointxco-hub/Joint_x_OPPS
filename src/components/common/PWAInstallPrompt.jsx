...
export default function PWAInstallPrompt() {
  ...
  if (!installEvent && online && queueCount === 0) return null;
      ...
      <div className="rounded-xl border border-border bg-card shadow-apple-lg px-4 py-3 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-foreground">
            {!online ? "Offline mode" : queueCount > 0 ? `${queueCount} waiting to sync` : "Install Joint X"}
          </p>
          <p className="text-xs text-muted-foreground">
            {!online ? "New expenses, notes, bugs and ideas will sync when online." : queueCount > 0 ? "Tap sync if it does not run automatically." : "Add it to your phone for faster access."}
          </p>
        </div>
        {queueCount > 0 ? (
